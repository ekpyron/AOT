...
lift_definition IsProperInX :: "(\<kappa>\<Rightarrow>\<o>)\<Rightarrow>bool" is
  "\<lambda> \<phi> . \<forall> x v . (\<exists> a . \<nu>\<upsilon> a = \<nu>\<upsilon> x \<and> (v \<Turnstile> \<phi> (a\<^sup>P))) = (v \<Turnstile> \<phi> (x\<^sup>P))" .
lift_definition IsProperInXY :: "(\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<o>)\<Rightarrow>bool" is
  "\<lambda> \<phi> . \<forall> x y v . (\<exists> a b . \<nu>\<upsilon> a = \<nu>\<upsilon> x \<and> \<nu>\<upsilon> b = \<nu>\<upsilon> y \<and> (v \<Turnstile> \<phi> (a\<^sup>P) (b\<^sup>P))) = (v \<Turnstile> \<phi> (x\<^sup>P) (y\<^sup>P))" .
lift_definition IsProperInXYZ :: "(\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<o>)\<Rightarrow>bool" is
  "\<lambda> \<phi> . \<forall> x y z v . (\<exists> a b c . \<nu>\<upsilon> a = \<nu>\<upsilon> x \<and> \<nu>\<upsilon> b = \<nu>\<upsilon> y \<and> \<nu>\<upsilon> c = \<nu>\<upsilon> z \<and> (v \<Turnstile> \<phi> (a\<^sup>P) (b\<^sup>P) (c\<^sup>P))) = (v \<Turnstile> \<phi> (x\<^sup>P) (y\<^sup>P) (z\<^sup>P))" .
...
named_theorems IsProperIn_defs
declare IsProperInX_def[IsProperIn_defs]
        IsProperInXY_def[IsProperIn_defs]
        IsProperInXYZ_def[IsProperIn_defs]
...
begin
  ...
  lemma D5_1[semantics]:
    ...
    using assms unfolding IsProperIn_defs semantics_defs
