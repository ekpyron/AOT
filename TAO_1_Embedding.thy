...
text{*
\begin{remark}
  Individual terms can be explicitely marked to represent
...
text{*
\begin{remark}
  An exemplification formula is only true if all individual variables denote.
...
lift_definition exe1::"\<Pi>\<^sub>1\<Rightarrow>\<kappa>\<Rightarrow>\<o>" ("\<lparr>_,_\<rparr>") is
  "\<lambda> F x w s . (denotes x) \<and> F (\<nu>\<upsilon> (denotation x)) w s" .
lift_definition exe2::"\<Pi>\<^sub>2\<Rightarrow>\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<o>" ("\<lparr>_,_,_\<rparr>") is
  "\<lambda> F x y w s . (denotes x) \<and> (denotes y) \<and>
     F (\<nu>\<upsilon> (denotation x)) (\<nu>\<upsilon> (denotation y)) w s" .
lift_definition exe3::"\<Pi>\<^sub>3\<Rightarrow>\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<kappa>\<Rightarrow>\<o>" ("\<lparr>_,_,_,_\<rparr>") is
"\<lambda> F x y z w s . (denotes x) \<and> (denotes y) \<and> (denotes z) \<and>
   F (\<nu>\<upsilon> (denotation x)) (\<nu>\<upsilon> (denotation y)) (\<nu>\<upsilon> (denotation z)) w s" .

subsection{* Encoding *}

text{*
\begin{remark}
  An encoding formula is again only true if the individual term denotes.
...
lift_definition enc :: "\<kappa>\<Rightarrow>\<Pi>\<^sub>1\<Rightarrow>\<o>" ("\<lbrace>_,_\<rbrace>") is
  "\<lambda> x F w s . (denotes x) \<and> case_\<nu> (\<lambda> \<omega> . False) (\<lambda> \<alpha> . F \<in> \<alpha>) (denotation x)" .
