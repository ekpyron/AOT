...
definition basic_identity\<^sub>E::"\<Pi>\<^sub>2" where
  "basic_identity\<^sub>E \<equiv> \<^bold>\<lambda>\<^sup>2 (\<lambda> x y . \<lparr>O!,x\<^sup>P\<rparr>  \<^bold>& \<lparr>O!,y\<^sup>P\<rparr> \<^bold>& \<^bold>\<box>(\<^bold>\<forall>\<^sub>1 F . \<lparr>F,x\<^sup>P\<rparr> \<^bold>\<equiv> \<lparr>F,y\<^sup>P\<rparr>))"
...
definition basic_identity\<^sub>\<kappa> (infixl "\<^bold>=\<^sub>\<kappa>" 63) where
  "basic_identity\<^sub>\<kappa> \<equiv> \<lambda> x y . (x \<^bold>=\<^sub>E y) \<^bold>\<or> \<lparr>A!,x\<rparr> \<^bold>& \<lparr>A!,y\<rparr> \<^bold>& \<^bold>\<box>(\<^bold>\<forall>\<^sub>1 F . \<lbrace>x,F\<rbrace> \<^bold>\<equiv> \<lbrace>y,F\<rbrace>)"
...
definition basic_identity\<^sub>2 :: "\<Pi>\<^sub>2\<Rightarrow>\<Pi>\<^sub>2\<Rightarrow>\<o>" (infixl "\<^bold>=\<^sub>2" 63) where
  "basic_identity\<^sub>2 \<equiv> \<lambda> F G .  \<^bold>\<forall>\<^sub>\<nu> x .((\<^bold>\<lambda>y.\<lparr>F,x\<^sup>P,y\<^sup>P\<rparr>) \<^bold>=\<^sub>1 (\<^bold>\<lambda>y.\<lparr>G,x\<^sup>P,y\<^sup>P\<rparr>)) \<^bold>&
                                    ((\<^bold>\<lambda>y.\<lparr>F,y\<^sup>P,x\<^sup>P\<rparr>) \<^bold>=\<^sub>1 (\<^bold>\<lambda>y.\<lparr>G,y\<^sup>P,x\<^sup>P\<rparr>))"
