...
AOT is a foundational metaphysical theory that explains the objects presupposed by the sciences as \emph{abstract objects} that reify property patterns.
SSEs are a means for universal logical reasoning that works by translating a target logic to HOL using a representation of its semantics.

In particular, we can verify Uri Nodelman's and Edward Zalta's reconstruction of Frege's theorem and confirm that the Peano-Dedekind postulates for natural numbers are derivable in AOT. Furthermore, we can suggest and discuss generalizations and variants of the construction and can thereby provide theoretical insights into, and contribute to the philosophical justification of, the construction.
...
During our work, we could significantly contribute to the evolution of our target theory itself, while simultaneously solving technical challenges of using an SSE to implement a theory that is based on significantly different logical foundations compared to the meta-logic HOL.
